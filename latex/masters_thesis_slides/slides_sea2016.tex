...
\node [above, font=\small, align=center] at (10.5, 1.5) {\(O(n)\)\\ \(\Theta(n)\)};
\def \origin {(0,-5)}
\drawhvectornotext{\origin}{20}{\rx}{\ry}{\scale}
...
\draw[above, font=\small, align=center, shift=\origin] node at +(11.5, 1.5) {\(O(n)\)\\ \(\Theta(n)\)};
...
\frame{\frametitle{All datasets together}
...
\item Combine the UKP5/`Ordered Step-Off' with MTU2? % as EDUK2 did with EDUK and MTU2
