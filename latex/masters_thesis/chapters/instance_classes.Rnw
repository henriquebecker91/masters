...
An instance of the classic CSP problem consists in a list of demands for \(m\) different cut sizes, that will be cut from the smallest amount necessary of master rolls with size \(c\). Each cut \(\)

The previously mentioned `set covering' formulation for BPP and CSP is a tight formulation proposed by Gilmore and Gomory in \cite{gg-61}. This formulation eliminated the problem of the classic formulation, that was loose and had too many symmetric solutions, but had an exponential number of variables.
...
subject~to & \sum_{j=1}^n a_{ij} x_j \geq b_i,~~~\forall i \in \{1,\dots,m\}  \label{eq:csp_demand}\\
           & x_j \in \mathbb{N}_0,~~~\forall i \in \{1,\dots,m\} \label{eq:csp_x_integer}
\end{align}
...
The author also finds important to show the risks of insufficient bibliographical research, and not re-evaluating the results of experiments presented in the past. In Section \ref{XXX}, we will see that simple DP algorithms of fifty years ago seems to be the best for solving the most recent benchmark dataset for UKP. 
