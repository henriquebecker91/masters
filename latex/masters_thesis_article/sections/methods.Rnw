...
To allow the reproduction of the experiments and also to contextualize them, this section describes the algorithms, their implementations, the datasets and the computer setup used.
...
The algorithms used in the experiments can pertain to three main approaches: dynamic programming (DP); branch-and-bound (B\&B); and hybrid (combine DP and B\&B).
...
The terminating step-off is a variant of the ordered step-off which executes exactly \(n\) and up to \(c\) extra operations to save up to \(c\) operations.
...
All C++ implementations written by the authors are available at the author's code repository\footnote{The C++ implementations of MTU1, MTU2, the ordered/terminating step-offs, and GREENDP are available at \url{https://github.com/henriquebecker91/masters/tree/136c1c1fbeb6ef7baa7ab6bcc8f48cb0bb68b697/codes/cpp}.}.
...
The authors' implementations of these three algorithms made use of a speed up trick (implementation detail) that has reduced considerably the run times for some instances, especially the instances of the subset-sum and strongly correlated datasets.
...
The trick consists in keeping the solution with the item of lowest index when the algorithm generates two or more solutions with the same weight and profit.
