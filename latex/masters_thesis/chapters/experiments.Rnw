...
The experiments used the reduced PYAsUKP benchmark (see Section \ref{sec:pya_dat_red_pya_dat}) and ten runs for each combination of the same algorithm, in the same computer, over the same instance, and in the same mode (parallell or serial).
