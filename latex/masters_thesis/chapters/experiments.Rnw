...
While the author don't examinated the PYAsUKP internals, he belives that the second PYAsUKP trend is formed by instances where the B\&B preprocessing phase (maybe together with the PYAsUKP dominance tests) had considerable success. This would also explain the lack of this trend at the sc and ss2 instance classes, where such bounds and tests have less effect (ss2, specifically, is comprised of subset-sum instances, that do not have different efficiency between items and, consequently, aren't affected by bounds). When PYAsUKP B\&B phase fails to yield good results, the PYAsUKP's DP phase times are greater than the UKP5 and mgreendp times.

The plateaus formed by the UKP5 and mgreendp runs show very little time variation between some groups of instances. A closer examination of the data reveals thos instance groups (plateaus) agrregate instances with the same number of items (for the same distribution) or, for some lower plateaus, instances with different number of items but all of them a magnitudes smaller than the numbers of items of the plateau(s) above. This behaviour shows that UKP5 and mgreendp1 are little affected by the specific items that make an instance, the instance size and distribution being good predictors of the time the UKP5/mgreendp will need to solve it. 

PYAsUKP seems to have a much greater variation between the times for instances with similar number of items of the same distribuion. We can see a line with a slope close to 45 degrees close to the top right corner of the charts. This line is over a logarithmic y axis, so it's a huge variation compared to the UKP5 plateau below it, that is solving the same instances (that, as we have seen above, have similar number of items).

The times between UKP5 and mgreendp are very similar, specially at instance classes hi, nsds2, where it has a small advantage. This should come with no surprise as they are similar algorithms. What is surprising is its behaviour at saw and sc instance classes, where the situation is reversed, and mgreendp takes considerably more time than UKP5 in many cases. The behaviour at those two classes can be explained by two main factors. The first factor is that mgreendp solves the knapsack subproblems without the best item, and regularly tests if it can abandon the DP and fill the remaining knapsack capacity with copies of the best item. The removal of the best item from the items list weakens the effect of the solution dominance applied by mgreendp (also used by UKP5). Some solutions that would be never generated by UKP5 will be generated at mgreendp because better solutions with the best item don't exist to dominate them. The second factor is the periodicity check applied by UKP5. This check has little overhead and stopped UKP5 computation before reaching the knapsack capacity the majority of the times (only 18 of 240 sc instances and 128 of the 1100 saw instances don't benefited from the periodicity check). % this data is buried in the file with all ukp5 outputs, sorry about that
