...
The experiments have shown that the standard deviation and the mean time of multiple runs of the same algorithm in the same computer vary considerably if they are executed in parallel or serially; how much they vary depends of the specific algorithm and computer. The experiments used the reduced PYAsUKP benchmark and ten runs for each combination of the same algorithm, in the same computer, over the same instance, and in the same mode (parallell or serial). For the same computer, all runs of one algorithm were executed before starting the first run of the other algorithms, what is specially important for the parallel runs, as this means they only have competed for memory with other runs of the same algorithm. The order of the runs between the ones with the same computer, algorithm and mode was random.

The author do not intend to do an in-depth analysis, but only to show that the difference exists and can be significative. The experiments described at Section \ref{XXX} were executed serially to avoid this noise. Also, this analysis answers a question of a referee made when the paper \cite{XXX} was under review, and show how the experiment setting (that corresponds to parallel runs over the \verb+notebook+ computer) benefited UKP5 over PYAsUKP. The author believes that the point made by \cite{XXX} still stands, as the effect over the numbers would not render a completely different analysis.
...
The SD of UKP5 is much smaller than the one of PYAsUKP (independent of mode or computer), what is probably caused by the locality of reference of the UKP5 algorithm in contrast with PYAsUKP. The desktop setting shows considerably less SD over the serial runs (in contrast to the notebook setting); what can be explained by the fact the L1 cache (that isn't shared between cores) is bigger in the desktop than the notebook computer. The desktop setting also shows considerably less SD for the parallel runs, however the instances with the biggest SD seems to be less affected by this effect than the others.
...
ggplot(t, aes(x = serial_time, y = ratio, color = inst_class)) + #scale_x_log10() +
...
Let's examinate closely the values for the notebook setting, as this was the one used at the \cite{XXX} paper. We will refer to UKP5 (or PYAsUKP) Parallel (or Serial) Total time (or TT, for short) as the sum of the internal times of all the runs of that algorithm in that computer and mode. The 

<<env_infl_nums,echo=true>>=
...
The author hypothesize that the cache growth means little to runs that even with the cache growth will not access the same region 
