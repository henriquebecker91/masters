...
\subsection{Data and code related to CSP pricing subproblem dataset}
...
ss2 <- ft %>%
  ...
  group_by(algorithm) %>%
  summarise(
    avg = sprintf("%.2f", mean(internal_time)),
    sd  = sprintf("%.2f",   sd(internal_time)),
    max = sprintf("%.2f",  max(internal_time)))
ss2[] <- lapply(ss2, function(x) gsub('NA', '--', x))
...
\hline
\multicolumn{3}{l}{Instance desc.} & \multicolumn{3}{l}{TSO} & \multicolumn{3}{l}{MGREENDP} & \multicolumn{3}{l}{PYAsUKP}\\
...
\multicolumn{3}{c}{See section~\ref{sec:subsetsum}} & \Sexpr{make_pya_row(ss2)}\\ % & 0.05 & 0.12 & 0.74 & -- & -- & -- & 2.52 & 21.75 & 302.51 \\
\hline
...
\multicolumn{3}{c}{See section~\ref{sec:subsetsum}} & \Sexpr{make_mtu_row(mt_ss2)}\\ %0.04 & 40 & 0.04 & 40 & 0.00 & 40 & 154.97 & 8\\
\hline
...
# "The results for the step-offs were ommited as they are very similar to the MGREENDP results."
...
cat('Dataset  & N & finished & \\makecell{Master\\\\time} & \\makecell{Pricing\\\\time} & \\makecell{Number of\\\\sub-problems} & \\makecell{Pricing time\\\\per subproblem}\\\\\\\\')
#cat('\\hline\n')
