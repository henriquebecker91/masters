...
\begin{figure}[!htbp]
\caption{Time spent solving each individual pricing problem generated by instance 1002\_80000\_DI\_12.txt (ANI\&AI dataset) with MTU1 and TSO. The MTU1 run was killed by timeout, TSO run did not.}
\begin{center}
...
However, between MTU1 and the four OSO variants, no solution of the continuous relaxation (number of rolls needed) differed more than a \(2^{-18}\) fraction of a roll.
Solving pricing problems using the multiplicative factor to work over integer profits seems to be a numerically stable approach.
...
@
The distribution of the total time spent solving pricing problems for each BPP/CSP instance was similar between the four OSO variants.
However, their mean time differed significantly: \(1.56\)s (sort, floating point), \(1.21\)s (no sort, floating point), \(1.46\)s (sort, integer), \(1.06\)s (no sort, integer).
...
The mean time reduction observed in `no sort' variants did not come from cutting the time spent sorting the items (less than 1\% of the total pricing time), but from how OSO behaved with a differently sorted list.
