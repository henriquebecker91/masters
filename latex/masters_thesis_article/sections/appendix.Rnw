...
# TODO: check why this isn't working for 
get_wmin <- function(row) {
  if (row[8] == 'hi') {
    row[9]
  } else {
    as.numeric(gsub(".*wmin([1-9][0-9]+).*", "\\1", row[2]))
  }
}
...
ft <- ft %>%
  ...
  summarise(
    avg = round(mean(internal_time), digits = 2),
    sd = round(sd(internal_time), digits = 2),
    max = round(max(internal_time), digits = 2))
...
make_pya_row(sc, a == -5 & n == 5000 & wmin == 10000)
@
...
\cline{1-3}\cline{4-6}\cline{7-9}\cline{10-12}
 5 & 5  & 10 & \Sexpr{make_pya_row(hi, a == 5 & n == 5000 & wmin == 10000)}\\
...
slow <- read.csv("../data/mtus_pya.csv", sep = ";")
slow$X <- NULL
