...
This behaviour shows that UKP5 and MGREENDP1 are little affected by the specific items that make an instance, and that the instance size and distribution are good predictors of the time the UKP5/MGREENDP will need to solve it.
...
This wasn't done because of the following reasons: many knapsacks are small and solved very fast, and in these cases the PYAsUKP timing reports zero or inaccurate values; also, the solutions returned by PYAsUKP should affect the next knapsacks generated (what can't happen in such setting), so the comparison would be unfair.
