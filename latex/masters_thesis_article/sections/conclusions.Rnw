...
It was not considered if the characteristics that make it harder also negatively affect the algorithms excluded from comparison, or if they are algorithm or approach-specific.
...
Not specifically EDUK, but any optimized DP algorithm, would outperform a B\&B algorithm in the context of a dataset that is hard to solve by the B\&B approach.
...
If they were the only instances considered, then MTU2 would be considered the best algorithm, however, considering all remaining datasets, MTU2 often come last.
The BREQ instances do not suffer from the same richness of simple, multiple and collective dominated items that led uncorrelated and weakly correlated instances to be critisized and abandoned.
However, they are very rich in threshold dominance, and 
