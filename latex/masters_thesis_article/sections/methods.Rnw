...
An often mentioned UKP application is to solve the pricing subproblems generated by the continuous relaxation of the Set Covering Formulation  (SCF) for the Bin Packing Problem (BPP) and Cutting Stock Problem (CSP) using the column generation approach~\cite[p. 455--459]{book_ukp_2004}\cite{gg-61}.
