...
The experiment described in this section is different than the previous experiments.
All other experiments consisted in UKP's instances of specific distributions saved in files, and executables that read those instances, solved them and returned the solving time.
In this experiment, the instances are not of the UKP but of the BPP and CSP.
The times presented are the sum of the times used to solve all the pricing subproblems (or master problems) generated while solving the continuous relaxation of the instances.
...
The inner working of floating point numbers favored the choice of potency of two.
...
The UKP5 sorts the items by non-increasing efficiency, but it works with any ordering.
...
The author found interesting to execute the two UKP variants with sorting enabled and disabled, to check if the sorting would pay off.
...
The CPlex was already being used for solving the master problem, taking this in account, the author choose to use it to solve the pricing problem too, as a baseline.
...
That is what is done in this experiments section.

The timeout used in those experiments was of 600 seconds. This choice was based in the number of CSP's instances, and that each one of them generate many pricing problems. However, after the results, the choice of timeout was clearly too conservative. The author plan to remake the experiments with a greater timeout before publishing a paper based in this thesis.
...
Consequntly, in the implementations of this experiment, those items are removed before solving the pricing problem with a non-CPLEX solver (CPLEX have no problem with such nonpositive profits).
The indexes of the items in the pricing problems' solutions are remaped to their old indexes before returning the solution to the master problem.
The same process of remaping is already done for algoriths that change the items' order.
The time of all remapping or conversion procedures is always added to the times taken by the algoriths to solve the pricing subproblems.
...
However, the pricing subproblems can be different for different methods over the same instance.
...
The chosen optimal solution will affect the master problem, that will generate a slight different knapsack subproblem.
This effect cascades and can change the profit values of all next knapsack subproblems, and the number of knapsack subproblems that are needed to solve (that's the same as the number of iterations).

The cplex\_cutstock stands out having many of the smallest number of iterations.
The author can't explain what property CPlex knapsack solutions have that create such a difference.
...
The UKP5 variants always return an optimal solution with the smallest weight possible, this translates to generating patterns with the greatest possible waste (the gap between optimal solution weight and the knapsack capacity equals to the waste in a CSP pattern).
The author's hypothesis is that such characteristic can affect negatively the number of iterations needed to find a pattern set that can't be improved (loop end condition).
...
The tie breaker of the integer method choose a different optimal solution, and started a cascade effect (one different solution from the knapsack solver is sufficient to change all subsequent knapsacks generated by the master problem solver).
