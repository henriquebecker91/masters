...
The difference between the fmtu and cpp-mtu: cpp-mtu will check if the vector is ordered by efficiency before running, and any subset-sum instance will be considered previously ordered. This impacts the difference between the two, maybe it's unfair to use cpp-mtu over subset-sum instances.

The specific method of ordering the 100 more efficient, one after other, is also different.
