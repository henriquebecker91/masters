...
OCaml code (PYAsUKP/EDUK/EDUK2) was compiled with ocamlopt and the flags suggested by the authors of the code for maximum performance (\emph{-unsafe -inline 2048}).
Fortran code (original MTU1/MTU2) was compiled with gcc-fortran and \emph{-O3 -std=f2008} flags enabled.
C++ code (all remaining implementations) was compiled with gcc and the \emph{-O3 -std=c++11} flags enabled.
...
The terminating step-off is a variant of the ordered step-off which executes exactly \(n\) and up to \(c\) extra operations in order to save up to \(c\) operations.
...
The authors' implementations of these three algorithms made use of an speed up trick (implementation detail) that has considerable positive effect over the run times of some instances, specially the strongly correlated intances.
...
The authors found redudant to present weakly correlated datasets in addition to the strongly correlated datasets, as preliminary results suggested that weakly correlated datasets times were similar to strongly correlated datasets of smaller size.
