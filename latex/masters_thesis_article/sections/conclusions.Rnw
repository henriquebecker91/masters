...
Except for the BREQ dataset, a revisited version of a DP \textcolor{blue}{algorithm} from 1966 (Revisited Terminating Step-Off, R-TSO) had lower mean times than the only known implementation of the current state-of-the-art algorithm (EDUK2 /PYAsUKP).
...
The authors believe TSO was not considered in recent comparisons because: the \textcolor{blue}{algorithm} was very similar to the naïve DP for the UKP but the authors did not emphasize it was orders of magnitude faster; B\&B algorithms performed better than TSO in uncorrelated and weakly correlated instances with one hundred items.
...
Such instances are now too small to consider, and the relative difference between TSO and MTU1 mean times was less than one order of magnitude apart \textcolor{blue}{and diminishing.
...
If they were the only instances considered, then MTU2 would be considered the best algorithm \textcolor{blue}{(as it can be seen in \autoref{tab:one_table}).}
...
\caption{
\textcolor{blue}{The number of \textbf{fin}ished runs of a method (row) over a dataset (column), and the \textbf{av}era\textbf{g}e time (in seconds) of the finished runs (dashes mean the respective method was not used to solve the respective dataset).
...
\textcolor{blue}{vi) CPLEX has better performance than B\&B algorithms for UKP in instances with few items but `hard' to solve by B\&B approach, but it is worse when the instances are large but `easy' (many variables) or too many small instances (costly initialization).}
...
\item \textcolor{blue}{The difference in performance of MTU1/MTU2 and CPLEX/Gurobi over the PYAsUKP dataset comes from the fact MTU1/MTU2 are depth-first? What would be the performance of a best-bound B\&B made for the UKP?}
\end{itemize}
