...
This behavior shows that the specific items that constitute an instance affect TSO and GREENDP less than the number of items and distribution, both which are good predictors of the TSO and GREENDP run time.
