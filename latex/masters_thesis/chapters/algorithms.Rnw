...
The objectives of this chapter are: to present relevant detail that didn't fit in the literature review; to further develop what was said last section, about that some approaches favoring some item distributions; to give the readers a base to understand the results of the experiments in Section~\ref{sec:exp_and_res}; and to explain the concept of solution dominance mentioned in Section~\ref{sec:dom_rel}.
The objective isn't to present an exhaustive list of approaches and algorithms.
...
However, based in the rationale exposed in the last paragraph, the author believes it's safe to assume that, for the same instance of the UKP, and the same solving approach (DP, B\&B, \dots), a state-of-the-art algorithm for the UKP will outperform a state-of-the-art algorithm for the BKP.
...
The DP approach often gives an optimal solution for each capacity smaller then~\(c\).
However, some space optimizations can remove such feature.

\subsection{The naive algorithm}

In this work, the author sometimes refer to the naive (or basic) DP algorithm for the UKP.
...
This algorithm will always execute about~\(n\times c\) operations (the worst case).
...
The procedure to recover the items that comprise the optimal solution will not be given for this and the remaining DP algorithms because, in general, it's the same procedure explained in UKP5 (see Section \ref{sec:ukp5}).
...
The condition can also be seen as: add a new item to a pre-existing solutions only and only if the new item is the most efficient item already in the solution, or an item even more efficient.
...
\subsection{UKP5 -- The proposed algorithm}
\label{sec:ukp5}
...
\caption{The solutions \(\{1,2\}\) and \(\{2,1\}\) (with \(w_1 = 3\) and \(w_2 = 4\)) are equivalent. The solution \(\{3,2,1\}\) can be also constructed by many ways (with \(w_1 = 4\), \(w_2 = 5\) and \(w_3 = 6\)). The figure shows the solution generation with and without symmetry pruning.}
\label{fig:symmetry}
\end{figure}
...
The solution's assembly phase isn't described in Algorithm~\ref{alg:ukp5}, but it's similar to the one described in~\cite[p. 221, Steps 6-8]{garfinkel}, and can be used for the already described Algorithms~\ref{alg:naive_dp} and~\ref{alg:gar_dp}.
...
This phase has a~\(O(c)\) time complexity, as a solution can be comprised of~\(c\) copies of an item~\(i\) with~\(w_i = 1\).
...
We use the~\(min_{ix}(s)\) notation to refers to the lowest index among the items that compose the solution~\(s\).
...
For example, if~\(\{3, 2\}\) is dominated, then~\(\{3, 2, 2\}\) and~\(\{3, 2, 1\}\) will never be generated by UKP5, but~\(\{3,2,3\}\) or~\(\{3,2,5\}\) could yet be generated (note that, in truth, are the equivalent~\([3,3,2]\) and~\([5,3,2]\) that could yet be generated).
Ideally, any~\(u\) where~\(t \subset u\) should not be generated as it will be dominated by a solution~\(u'\) where~\(s \subset u'\) anyway. 
It's interesting to note that this happens eventually, as any~\(t \cap \{i\}\) where~\(i > min_{ix}(t)\) will be dominated by~\(s \cap \{i\}\) (or by a solution that dominates~\(s \cap \{i\}\)), and at some point no solution that is a superset of~\(t\) will be generated anymore.
...
The MTU1 algorithm is a B\&B algorithm for the UKP that avoids the explicit unfolding of the typical B\&B tree, it was proposed in~\cite{mtu1}.
...
In the algorithm's description, it's to be assumed that the items are ordered in the mentioned order
...
For each item~\(j\) that's not in the core problem, an upper bound is computed over the solutions with one single copy of item~\(j\), if this upper bound is equal to or smaller than the value of the lower bound, then the item can be discarded without loss to the optimal solution value.
...
The implementation of the GREENDP made by the author of this thesis, and used in the experiments section, will be called MGREENDP (Modernized GREENDP, in the sense that the algorithm now uses loops instead of the \verb+goto+ directive). 

The GREENDP algorithm consists in solving the UKP using the ordered step-off algorithm, but without using the best item in the DP, and with interruptions at each~\(w_b\) capacity positions, for checking if the DP can be stopped and the remaining capacity filled with copies of the reserved best item.
...
A weakness of this bounds calculation is that it fails automatically if the two most efficient items have the same efficiency.
...
The hybridized EDUK algorithm works like EDUK would, with the addition of an extra phase between the slices.
