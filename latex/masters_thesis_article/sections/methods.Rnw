...
The datasets used in the experiments include: some datasets from the literature that mainly focus on being hard-to-solve (...); one dataset proposed by the authors to make a point (...); one dataset CONTINUE HERE
The reasoning for not including \emph{uncorrelated} and \emph{weakly correlated} instances is also presented.
...
The PYAsUKP dataset consists of at least ten instances with different random seeds for each combination of the remaining generation parameters.
The authors will refer to one tenth of the PYAsUKP dataset (454 instances), consisting of at least one instance with each combination of non-seed parameters, the \emph{reduced PYAsUKP dataset}.
...
% TODO: consider reducing the paragraph below
