...
\hline
\multicolumn{3}{l}{Instance desc.} & \multicolumn{3}{l}{TSO} & \multicolumn{3}{l}{GFDP} & \multicolumn{3}{l}{PYAsUKP}\\
...
\end{tabular}
\end{adjustbox}
\caption{Time in seconds for the MTU implementations over the reduced PYAsUKP's dataset (see Section \ref{sec:mtu_exp}). Columns \textbf{n} and \(w_{min}\) values must be multiplied by \(10^3\) to obtain their true value. Let \(T\) be the set of run times reported by CPP-MTU1, CPP-MTU2, F77-MTU1 and F77-MTU2, for the instance dataset described by a row (in this case, we don't count runs that ended in timeout). The meaning of the columns \textbf{avg} and \textbf{fin}ished, is, respectively, the arithmetic mean of \(T\) and the cardinality of \(T\). The time limit was set to 30 minutes. For the subset-sum instances, there is one instance for each possible combination of: \(w_{min} \in \{10^3, 5\times10^3, 10^4, 5\times10^4, 10^5\}\); \(w_{max} \in \{5\times10^5, 10^6\}\) and \(n \in \{10^3, 2\times10^3, 5\times10^3, 10^4\}\), totaling 40 instances.}
\end{table}
...
ct <- inner_join(pya_solvers, opts, by = 'filename')
...
\multicolumn{3}{l}{40 inst. per line} & \multicolumn{6}{l}{Subset-sum. Random \emph{c} between \([5\times10^6; 10^7]\)}\\
\cline{1-3}\cline{4-9}

& \textbf{n} & \(w_{min}\) & \textbf{avg} & \textbf{fin}  & \textbf{avg} & \textbf{fin} & \textbf{avg} & \textbf{fin} & \textbf{avg} & \textbf{fin}\\
\cline{1-3}\cline{4-6}\cline{7-9}

\multicolumn{3}{c}{See caption.} & \Sexpr{make_mtu_row(mt_ss2)}\\
\hline

\multicolumn{3}{l}{20 inst. per line} & \multicolumn{6}{l}{Strong correlation. Random \emph{c} between \([20\overline{n}; 100\overline{n}]\)}\\
\cline{1-3}\cline{4-9}
...
\multicolumn{3}{l}{20 inst. per line} & \multicolumn{6}{l}{Postponed periodicity. Random \emph{c} between \([w_{max}; 2\times10^6]\)}\\
\cline{1-3}\cline{4-9}
...
\multicolumn{3}{l}{50 inst. per line} & \multicolumn{6}{l}{No collective dominance. Random \emph{c} between \([w_{max}; 1000\overline{n}]\)}\\
\cline{1-3}\cline{4-9}
& \textbf{n} & \(w_{min}\) & \textbf{avg} & \textbf{sd} & \textbf{fin} & \textbf{avg} & \textbf{sd} & \textbf{fin}\\
\cline{1-3}\cline{4-6}\cline{7-9}
...
\multicolumn{3}{l}{\emph{qtd} inst. per line} & \multicolumn{6}{l}{SAW. Random \emph{c} between \([w_{max}; 10\overline{n}]\)}\\
\cline{1-3}\cline{4-9}
...
\end{tabular}
\end{adjustbox}
\caption{Time in seconds for the model implementations (CPLEX and Gurobi) over the reduced PYAsUKP's dataset (see Section \ref{sec:sec:cplex_gurobi_exp}). Columns \textbf{n} and \(w_{min}\) values must be multiplied by \(10^3\) to obtain their true value. Let \(T\) be the set of run times reported by CPLEX and Gurobi, for the instance dataset described by a row (in this case, we don't count runs that ended in timeout, memory exhaustion or with a wrong answer). The meaning of the columns \textbf{avg}, \textbf{sd} and \textbf{fin}ished, is, respectively, the arithmetic mean of \(T\), the standard deviation of \(T\), and the cardinality of \(T\). The time limit was set to 30 minutes. For the subset-sum instances, there is one instance for each possible combination of: \(w_{min} \in \{10^3, 5\times10^3, 10^4, 5\times10^4, 10^5\}\); \(w_{max} \in \{5\times10^5, 10^6\}\) and \(n \in \{10^3, 2\times10^3, 5\times10^3, 10^4\}\), totaling 40 instances.}
\end{table}
