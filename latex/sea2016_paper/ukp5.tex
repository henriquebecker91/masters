...
It tries to solve the problem by B\&B, and if it fails to solve the problem quickly, it switches to DP using some data gathered by the B\&B to speed up the process. 
...
The sum of the items weight or profit of a solution \(s\) is denoted by \(w_s\) and \(p_s\), respectively.
...
If two or more items have the same weight we consider only the one with the best profit (the other can be discarded without losing the optimal solution value), if they have the same weight and profit we consider them the same item.
...
These two dominances are too time demanding to be applied at a preprocessing phase, while simple and multiple can. 
They are often integrated in the UKP algorithm, and remove items while the algorithm executes. 
The collective dominance needs to know the \(opt(y)\) to exclude an item \(i\) with \(w_i = y\), where \(opt(y)\) is the optimal solution for a capacity $y$.
...
The $y^*$ is \(O(1)\) on an item list ordered by non-increasing efficiency,  and it is generic, being successful applied on instances of most classes.
Assuming $i$ is the best item, and $j$ is the second best item, then \(y^{*} = p_i / (e_i - e_j)\).
...
\begin{enumerate}
  ...
  \item Sparsity: not every position of the capacity array has to be computed;
  \item Dominated solutions pruning: dominated solutions are pruned;
  \item Time/memory tradeoff: the test \(w_i \leq y\) from the algorithm in~~\cite[p. 221] was removed in cost of more O($w_{max}-w_{min}$) memory;
  \item Periodicity: the periodicity check suggested in~\cite{gar72} (but not implemented there) was used.
\end{enumerate}
...
\Procedure{UKP5}{$n, c, w, p, w_{min}, w_{max}$}
  ...
  \For{\(y \gets w_{min}, c-w_{min}\)}\label{main_ext_loop_begin}\Comment{Can end early because periodicity check}
    \If{\(g[y] \leq opt\)}\label{if_less_than_opt_begin}\Comment{Removal of dominated solutions}
...
%Any reference to specific lines until this section end refer to the lines presented in Algorithm \ref{alg:ukp5}.
...
The solution assemble phase isn't described in algorithm \ref{alg:ukp5}. 
But it's similar to the one used by the DP method described in \cite[p. 221, Steps 6-8]{gar72}. 
We add a copy of item \(i = d[y_{opt}]\) to the solution, then we add a copy of item \(i_2 = d[y_{opt} - w_i]\), and so on, until \(d[0]\) is reached. 
...
The solution dominance is applied in lines \ref{if_less_than_opt_begin} to \ref{if_less_than_opt_end}, mainly by
the use of \(d[y]\) in line \ref{main_inner_loop_begin}. 
%It's affected, but not dependent on, the order of the item list. 
...
This is also true for any solution \(\{3, 2\} \cap v\), where \(v\) has only indexes lower than or equal to \(2\). 
...
This periodicity check works only if the best item is the first item. 
...
Nonetheless, without this test, the UKP5 was about 1800 (one thousand and eight hundreds) times slower on the subset-sum instances presented at~\ref{tab:times}. For other instance classes, no significant speedup was observed.
...
We use the following notation: \(rand(x, y)\) means a random integer between \(x\) and \(y\) (both inclusive); \(x\overline{n}\) means \(x\) as a string concatenate with the value of variable \(n\) as a string. For example: if \(n = 5000\) then \(10\overline{n} = 105000\).
...
The PYAsUKP parameters \emph{-wmin \(w_{min}\) -cap c -n \textbf{n}} were used in all instances generation. 
...
Table \ref{tab:times} presents the times used by UKP5 and PYAsUKP to solve the instance classes previously described. No time limit was defined.
...
%., in a way that no dynamic programming algorithm can be competitive against. 
...
All other datasets have instances that present the same characteristics, and because of that, the PYAsUKP minimal time is always close to zero. 
...
For the presented classes, the ratios $\frac{no-B\&B avg time}{B\&B avg time}$ by instance class are: 
