...
The MTU2 algorithm calls MTU1 over the \(x\%\) most efficient items, and if this is not sufficient to obtain a solution with proved optimality, it repeat the process with \(x\%\) increased~\cite{mtu2}.
