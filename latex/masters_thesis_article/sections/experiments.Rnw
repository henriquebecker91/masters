...
TSO and MGREENDP run times form plateaus in the figure.
...
This four implementations were executed over the reduced PYAsUKP benchmark.
...
The subset-sum instances were the ones that exhibited the greatest difference.
...
In instances with a small number of items, MTU2 behaves almost the same way that MTU1, the same applies to the TSO e OSO.
...
In the majority of these easy datasets, the highest times presented are from MTU1, and OSO presented the lowest mean time.
