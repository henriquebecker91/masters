...
In another words, it's a guarantee that any optimal solution for an instance where \(y^{*} \leq c\) has at least one copy of the best item. 
...
This way, for any UKP instance where \(y^{*} \leq c\) we can reduce instance capacity by \(max(1, \lceil(c-y^{*})/w_b\rceil)\times w_b\). After solving this instance with reduced capacity we can add \(max(1, \lceil(c-y^{*})/w_b\rceil)\) copies of \(b\) to the optimal solution to obtain an optimal solution for the original instance.
...
What we are saying is that, as the algorithm executes, some items will stop being used to create new solutions, as the four known dominance relations were being applied to the item list, with little, if any, overhead.
