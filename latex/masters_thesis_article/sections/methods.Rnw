...
In the enumeration tree of MTU1, each node at depth~\(0 \leq d < n\) has~\((c'/w_{d+1}) + 1\) childs (where \(c'\) is the remaining capacity at current node), with each of those childs representing a valid amount of copies of the item \(d\) being packed in the knapsack (including zero copies).
...
The states, stages and basic recursion used by DP algorithms for the UKP are similar and generically described below.
An advantage DP algorithms for the UKP have against DP algorithms that solve BKP and 01 KP is that the states consist solely of weight and profit pairs; which items were already used do not affect the validity of the solution. 
Consequently, there is no need for separating states by the items used, and if two or more states have the same weight only one needs to be stored, allowing DP algorithms to use \(O(c)\) memory instead of \(O(nc)\).
...
Its complexity is justified by the fact Sage-3D does not solve the UKP for a specific knapsack capacity, but instead explores periodicity to build a data structure which allows querying the solution for a specific capacity in \(O(log(p_b))\) (polynomial in the input size).
