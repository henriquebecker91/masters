...
Some algorithms were implemented, but not used at this benchmark. Those are the two algorithms from \cite{XXX}, the algorithm from \cite{XXX} (mgreendp, from Section \ref{XXX}), the MTU2 algorithm and EDUK/EDUK2 (PYAsUKP).
...
This benchmark needs to call the UKP solving methods from inside the C++ code with the CPLEX master's model. The author tried to integrate EDUK and EDUK2 (that are written in OCaml) to the benchmark, using the interface between C/C++ and OCaml, but the examples provided in PYAsUKP for this kind of integration weren't working. The author could have saved the knapsack instances generated when solving the CSP problem with other algorithm, and solved them using the PYAsUKP executable. This wasn't done because of the following reasons: as we will see, the result given by the knapsack solver affects the form and quantity of the next knapsack instances, so the comparison would be unfair; second, many knapsacks are small and solved very fast, and the PYAsUKP executable timing is not very precise.
...
In the section \ref{XXX} (Future Works) we will see some questions raised by this conclusion. Although there was one outlier, seems like converting profit values to integer seems to be a valid method to test classic UKP algorithms (that work only with integer profits) for solving CSP subproblems.
