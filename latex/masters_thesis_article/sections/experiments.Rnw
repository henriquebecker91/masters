...
In order to study this behaviour, let us analyse the times of individual pricing problems of the instance 1002\_80000\_DI\_12.txt (ANI\&AI dataset).
The pricing problems generated by the same BPP/CSP instance always share the same \(n\), \(c\) and items weights, the only significant difference between them is the item profit values\footnote{As the specific code used removes items with nonpositive profit values before the beggining of the algorithms, \(n\) and item weights do vary, but in a way not significantly different than the way they would vary in an algorithm that applies simple dominance over the item list before it starts.}.
The pricing problems generated when solving this specific instance have \(n = 911\) and \(c = 66432\).

\begin{figure}[!htbp]
\caption{Time spent solving each individual pricing problem generated by instance 1002\_80000\_DI\_12.txt (ANI\&AI dataset) with MTU1 and TSO. The MTU1 run was killed by timeout (30 minutes), TSO run did not.}
\begin{center}
<<csp_inst, fig.height = 4, eval = F>>=
...
As patterns including items of different sizes are added, the profit of the items seem to approximate \(\frac{c}{w_i}\).
...
The OSO algorithm sort the items by nonincreasing efficiency, but does not depend in this ordering to work.
In order to verify if the sorting cost would pay off, variants with sorting enabled and disabled were used.
Consequently, four versions of the OSO were used in this experiment, for all combinations of profit type (the original floating point, or the converted integer), and sorting (sorting by nonincreasing efficiency, or not sorting, which is the same that having the items sorted by increasing weight).

<<check_all_opt, results = 'hide'>>=
{
  # input: a dataframe with columns filename, algorithm and opt
  # output: a logical value
  # what it does: it checks if all non-NA opt values of diferent
...
In this experiment, the difference in exactly which optimal solution is returned for a pricing subproblem (because of different tie-breaking or because floating point innacuracy) changes the next pricing problem, and which are its optimal solutions, in a feedback loop.
...
Solving pricing problems using the multiplicative factor to work over integer profits seems to be viable approach.%a numerically stable approach.
...
@
%The time spent solving all pricing problems of an specific BPP/CSP instance  the four OSO variants. %The distribution of the total time spent solving pricing problems for each BPP/CSP instance was similar between the four OSO variants.
