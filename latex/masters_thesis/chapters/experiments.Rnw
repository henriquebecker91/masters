...
The fourth experiment execute four variants of the UKP5, MTU1 and CPLEX to solve the pricing subproblem described in Section~\ref{sec:csp_ukp_inst}. The fifth and last experiment addresses some concerns raised by the comments of an anonymous referee in the peer review of~\cite{sea2016}. 
The concerns are related to the effects of the shared memory cache in the comparison of the algorithms runtimes, when more than one run is executed at the same time.
...
The subset-sum (S.S.) distribution does not show MGREENDP data points (see Figure \ref{fig:pya_fast}).
...
Figure~\ref{fig:pya_fast} shows that, except for Strong Correlated (S.C.) and S.S., the runtimes of the PYAsUKP runs often show two main trends.
The first trend starts at the left of the x axis and covers UKP5 and MGREENDP (i.e. no run of UKP5 or MGREENDP has a time above this trend).
The second trend begins at middle/right of the x axis and is comprised of runtimes significantly smaller than UKP5 and MGREENDP.
Both trends merge in a single trend that points to the top right corner of the chart.
UKP5 and MGREENDP present similar runtimes that form plateaus.
...
When PYAsUKP B\&B phase do not solve the instance or help to greatly reduce the amount of states, the PYAsUKP DP phase times are greater than the UKP5 and MGREENDP times.
...
This behaviour shows that UKP5 and MGREENDP1 are little affected by the specific items that constitute an instance, and that the instance size and distribution are good predictors of the time the UKP5/MGREENDP will need to solve it.
...
For specific instances, however, PYAsUKP can show times much smaller, similar or much greater than UKP5.
As PYAsUKP times seem to grow more than UKP5/MGREENDP times with the instance growth, the possibility that PYAsUKP has better results than UKP5 for even bigger instances of those distributions seem improbable (though possible).
UKP5 and MGREENDP have similar results, as expected.
For some distributions, UKP5 is slight worse than MGREENDP; for other distributions, UKP5 is considerable better than MGREENDP.
The choice between one and another has to be made based in the distribution (especially considering that MGREENDP will need workarounds to work with instances where the two most efficient items can have the same efficiency).
...
In this paper, the authors compare MTU2 to EDUK and EDUK2 by using an easier version of the PYAsUKP benchmark. % TODO: EASIER? What I was thinking? Reduced. Maybe only reference.
...
The author tried to execute MTU1 and MTU2 over the entire PYAsUKP dataset, however many runs were ended by timeout (runtime greater than 1000 seconds), and executing the four B\&B codes over the 4540 instances would take time from more relevant experiments.
...
One is to show that the implementations of MTU1 and MTU2 written by the author of this thesis, in C++, are on par with the original ones, written in Fortran77, and can be used in the place of them for the remaining experiments.
The second purpose is to complement the comparison presented in last section by showing that both algorithms (independent of the implementation) are not competitive for the PYAsUKP dataset. % TODO: TERRIBLY WRITTEN, competitive is wrongly used

The Fortran codes were not exactly the original MTU codes.
...
% TODO: is not this duplicated? the commit should be a footnote, if not put in the appendix
...
The Fortran version read the same instances from a simplified format (that preserved the order of the items inside the instance).
...
The reasons are: the C++ implementation can read the same format used by PYAsUKP (and the other algorithms); with the exception of PYAsUKP (which uses OCaml) all other methods use C++ and the same structures; choosing the Fortran implementation would considerably harm MTU2 runtimes for something that seems to be a minor implementation detail.
...
Let's examine Figure~\ref{fig:breq_bench}.
...
Among the pure DP algorithms, EDUK was the one with the worst runtimes, being clearly dominated by our two UKP5 versions. 
...
The second fact is not so surprising considering that the original dataset from \cite{survey2014}\footnote{The dataset used in this section was described in Section \ref{sec:csp_ukp_inst}. It is comprised by 10\% of the instances used in \cite{survey2014}, some gathered from datasets of the literature and some proposed in the just cited work.} included many old datasets, as it tried to be extensive; also, solving the continuous relaxation of a CSP instance takes considerably less effort than solving the problem itself.
...
After the publication of~\cite{sea2016}, the author realized that algorithm runtimes were affected by executing multiple runs in parallel, \emph{even if each run was being executed in an exclusive/isolated core}.
...
Only the UKP5 and the PYAsUKP (EDUK2) were used; both algorithms were present in the original comparison~\cite{sea2016}, and both algorithms are relatively fast (a practical concern). 
...
The experiments have shown that the mean time of multiple runs of the same algorithm in the same computer vary considerably if they are executed in parallel or serially; how much they vary depends on the specific algorithm and computer.
...
The author believes that the point made in~\cite{sea2016} still stands, as the effect found over the algorithms runtimes would not render a different analysis.
...
The result would be that more time is added to the PYAsUKP total time because of this effect than time is added to the UKP5 total time (in absolute numbers), making the comparison unfair to PYAsUKP.% (unless we are interested on how the algorithms behave in a parallel setting).
...
We will see that this considerable absolute difference does not end up benefiting UKP5 in the analysis, as the PYAsUKP Serial TT is about 15.33 times greater than the UKP5 Serial TT, but the PYAsUKP Parallel TT is about 12.10 times bigger than UKP5 Parallel TT.
In fact, the parallel execution seems to benefit the PYAsUKP numbers against UKP5 while not by much.
If we focused in the \verb+desktop+ setting, we would see that PYAsUKP was benefited too, while to a lesser degree.
In the \verb+desktop+ setting, the PYAsUKP Parallel TT is 15.62 times greater than the UKP5 Parallel TT; and the PYAsUKP Serial TT is 16.50 times greater than the UKP5 Serial TT.
