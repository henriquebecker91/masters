...
\Procedure{UKP5}{$n, c, w, p, w_{min}, w_{max}$}
  ...
  \For{\(y \gets w_{min}, c-w_{min}\)}\label{main_ext_loop_begin}\Comment{At \(c-w_{min}\), \(g[c]\) is refered for the last time}
    \If{\(g[y] \leq opt\)}\label{if_less_than_opt_begin}\Comment{Pruning of dominated solutions}
    ...
    \For{\(i=1,d[y]\)}\label{main_inner_loop_begin}\Comment{Creates new solutions (none symmetric)}
...
%        \State \(d[y + w_i] \gets i\)
  ...
  \EndFor\label{main_ext_loop_end}\Comment{Can end early because periodicity check explained later}
