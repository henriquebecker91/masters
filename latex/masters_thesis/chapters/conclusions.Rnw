...
\end{description}
It's very important to note that, taking in account the historical context, the choices taken by previous researchers were sound and reasonable.
...
A possibility that can only be considered more worrying by the fact that the algorithm's pseudocode is available in \cite{gg-66}, a paper that's cited by many of the works mentioned above.
...
The first involves keeping a index in each solution to mark which items can yet be added to the solution.
...
Maybe new implementations of the EDUK approach can show the superiority of applying the four dominances in the EDUK's fashion.
...
\begin{itemize}
\item Which other real-world problems have UKP as a pricing subproblem? There are real-world problems that can be modelled as UKP? Would instances of those problems be easy for the current state-of-the-art algorithm?
