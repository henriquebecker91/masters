...
The Fortran implementation only sorted the items by efficiency, the C++ implementation breaks efficiency ties sorting by weight.
...
All three algorithms store the index of the most efficient item of a solution when it is generated, and do not store two solutions with the same weight but only the one with the greatest profit between them.
The trick consists in keeping the solution with the most efficient item when two or more solutions with the same weight and profit are generated.
