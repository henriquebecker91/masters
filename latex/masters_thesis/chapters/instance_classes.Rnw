...
If the author of this thesis could suggest one single thing to be improved in future works about UKP, it would be the choice of the instances.
...
The instances with an uncorrelated items distribution are a family of UKP instances in which the weight and the profit of an item have no correlation.
The most common way of generating uncorrelated instances is generating a value between~\(w_{min}\) and~\(w_{max}\) for the weight, and a value between~\(p_{min}\) and~\(p_{max}\) for the profit, for each of the~\(n\) items of the instance using a Pseudo-Random Number Generator (PRNG) with a uniform distribution.
...
We has chosen to not use uncorrelated instances in our experiments.
The main reasons behind this choice are that: \emph{the times used by an algorithm to solve uncorrelated instances have little to do with the algorithm's capability of solving a NP-hard problem, and much to do with the polynomial algorithms or heuristics it uses to remove simple and multiple dominated items}.
...
The following subsections present a detailed critical analysis about two uncorrelated instance datasets used in the literature (and the parameters used to generate them).
...
All instances have more than 99.5\% probability\footnote{Calculated the same way, but using~\(\frac{9}{100} \times \frac{1}{2}\) as the chance of generating and item~\(i\) with~\(w_i \leq \frac{w_{max}}{10}\) and~\(p_i > \frac{p_{max}}{2}\).} of having an item~\(i\) with~\(w_i \leq \frac{w_{max}}{10}\) and~\(p_i > \frac{p_{max}}{2}\), that would multiple dominate all items~\(j\) with~\(w_j > 2 \times \frac{w_{max}}{10}\) (i.e. at least 80\% of the items are multiple dominated by a single item in all instances).
...
This section discusses the instance datasets proposed in~\cite{pya}, and reused in the comparison presented in~\cite{sea2016}.
...
These datasets are similar to the ones used in~\cite{pya}. The same tool was used to generate the datasets (PYAsUKP) and the same parameters were used, unless otherwise stated.
...
Also, when referring to the parameters for the generation of an instance,~\(w_{min}\) will be used to denote the smallest weight that can appear in an instance, but without guarantee that an item with this weight will exist in the generated instance.
...
The implementation of the EDUK and EDUK2 algorithm have imprecise time measuring, i.e. if the algorithm takes less than a second to solve an instance, the runtime returned by the program is sometimes zeroed. 
...
The PYAsUKP \emph{-form ss -wmax~\(w_{max}\)} parameters were used.
...
There is threshold dominance in such instances, as bigger items will probably dominate solutions comprised of many copies of a smaller item.
...
The dataset comprised by one tenth of the PYAsUKP dataset, following the same distribution, and totalling 454 instances, will be called for the rest of this work: \emph{the reduced PYAsUKP dataset}.
...
The solution of this pricing problem is the cutting pattern that, if added to the master problem, will give the greatest improvement to master problem optimal solution.
...
The solving process alternates between solving the master problem and the pricing problem, until all cutting patterns that could improve the solution of the master problem are generated and added to the master problem.
The profit values of the pricing problem (dual variables) are real numbers close to \emph{one}.
If the value of the optimal solution for the pricing problem is \emph{one} or less, we have a guarantee that the master problem cannot be improved by adding any new cutting patterns to it.
...
In the real-world, an implementation of the pricing problem can return a value slight above \emph{one}, when it should have returned \emph{one} or slight less than \emph{one}.
...
Taking this into account, a better method for stopping the computation is verifying if the current pricing problem is equal to the one from last iteration, or if the solution of the pricing problem is equal to the one from the last iteration.
...
For the same instance of the CSP, the number of UKP instances generated, and their exact profit values, can vary based on the choice of optimal solution made by the UKP's solver (for the same pricing problem many cutting patterns can be optimal, but only one among them is added to the master problem).
...
The author believes that these optimizations do not affect considerably the structure of the pricing problem.
...
A list of implementation details follows: cutting patterns that are not used by the last master problem's solution could be removed, however they can end up being valuable again in the future and re-generated by the pricing problem, so the author choose to not remove them; the classic polynomial method for creating the initial set of cutting patterns was used, it consists in creating~\(n\) patterns, each one with only one sheet size that is cut as many times as possible, state-of-the-art solvers often begin using cutting patterns generated by a more sophisticated heuristic; the sheet sizes can be divided in two groups, the half with more demand, and the other half, and the pricing problem can be restricted to the high demand group in some conditions -- this also was not done.
...
It makes use of the column generation approach, however the pricing problem is not exactly the UKP, as consequence of the way the algorithm adds cuts to the continuous relaxation.
...
It is important to point that this is an artificially generated distribution that does not model any real-world instances (that the author has knowledge).
The author discourages the use and study of such artificial instances (including this one), and created this distribution only as a tool to demonstrate that it is not hard to create a distribution that benefits a solving approach over another.
It is the author's opinion that operational research could focus more on real-world instances and/or artificially generated instances that clearly model real-world instances.
...
In this context,~\(w_{max}\) and~\(p_{max}\) define the quadrant top right corner, i.e. the possible maximum value for an item's weight and profit, an item with those exact values does not need to exist in a BREQ instance.
...
However, in many instances the knapsack capacity is smaller than those threshold values and therefore the threshold dominance is not applied or relevant.
On BREQ instances, as a consequence of the quadratic profit growth, an optimal solution will never include the item~\(i\) two or more times if there is an item~\(j\) such as that~\(\sqrt{2} \times w_i \leq w_j \leq 2 \times w_i\).
...
Any gap left will probably be filled by the heaviest item that fits the gap, with this process repeated until no item fits the gap left (or there is no gap).
The classical greedy heuristic procedure that follows those steps would probably yield an optimal solution.
However, this is not always the case\footnote{A counter-example follows: consider an instance with~\(n = 4\),~\(c = 512\),~\(w_1 = 384\),~\(p_1 = 2774\),~\(w_2 = 383\),~\(p_2 = 2756\),~\(w_3 = 129\),~\(p_3 = 265\),~\(w_4 = 32\) and~\(p_4 = 17\); the optimal solution do not use the best item (\(w_1, p_1\)); the best solution when using the best item has a profit value of~\(2842 = 2774 + 4\times17\) (weight~\(512 = 384 + 4\times32\)) while the best solution when using the second most efficient item has the optimal profit value of~\(3021 = 2756 + 265\) (weight~\(512 = 383 + 129\)).
...
This instance distribution will be called the BREQ 128-16 Standard, it is a BREQ distribution where~\(c = 128 \times n\),~\(p_{min} = w_{min} = 1\),~\(w_{max} = c\) and~\(p_{max} = 16 \times w_{max}\).
...
The BREQ instances are a special case, as their purpose is exactly showing that it is easy to design an items distribution that favors one approach over other.
