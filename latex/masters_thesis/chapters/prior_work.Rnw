...
Two years later, in~\cite{gg-63}, the authors proposed a specific algorithm for UKP, and experiments solving BPP and CSP instances were executed.
...
EDUK (\emph{Efficient Dynamic programming for the Unbounded Knapsack problem}), a novel DP algorithm for UKP, was proposed in a conference paper~\cite{ukp_new_results} and then presented in a journal paper~\cite{eduk}.
...
The comparison uses a completely different dataset of small instances, in an effort to take into account real-world applications of UKP, as the ones provenient from solving BPP and CSP with column generation.
...
However, the algorithm proposed in~\cite{babayev} considered performance as a priority, while other works treated applying CA to UKP as an interesting theoretical problem.
...
Before ending this literature review, we would like to discuss the chapters about UKP in the following textbooks:~\cite{tchu} and~\cite{garfinkel}.
...
The chapter about UKP in~\cite[p.~214]{garfinkel} has an extensive bibliographical review of the works about UKP that predates it (1972), which is very relevant as the name UKP was only established some years later.
The chapter also discusses in depth many of the approaches used to solve UKP in that period, yet making heavy use of mathematical notation.
