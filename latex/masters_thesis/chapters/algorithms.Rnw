...
In Section~\ref{sec:introduction}, it was pointed out that UKP can be seen as a special case of BKP where, for each item type~\(i\), there are at least~\(\floor{\frac{c}{w_i}}\) copies of that item type available in an instance.
...
The procedure to recover the items that comprise the optimal solution will not be given for this and the remaining DP algorithms because, in general, it is the same procedure explained in UKP5 (see Section \ref{sec:ukp5}).
...
The condition can also be seen as: add a new item to a pre-existing solution only and only if the new item is the most efficient item already in the solution, or an item even more efficient.
...
The solution's assembly phase is not described in Algorithm~\ref{alg:ukp5}, but it is similar to the one described in~\cite[p. 221, Steps 6-8]{garfinkel}, and can be used for the already described Algorithms~\ref{alg:naive_dp} and~\ref{alg:gar_dp}.
...
We use the~\(min_{ix}(s)\) notation to refer to the lowest index among the items that compose the solution~\(s\).
...
This may seem unnecessary, as appears to be an optimization of a rare case, where two solutions comprised of different item multisets have the same weight and profit. 
...
The B\&B approach was established in the seventies as the most efficient approach for solving the UKP\footnote{To the author's knowledge this claim was first made in~\cite{mtu2}, and after this other papers as~\cite{babayev} began repeating it.
The fact that only the code for MTU1 and MTU2 was readily available also did not help the situation, as some began to claim that MTU2 was the \emph{de facto} standard algorithm for UKP, see~\cite{ukp_new_results}.}, what is greatly a consequence of the datasets, items distributions and/or generation parameters used at the time (that favored B\&B in the experiments).
...
The time taken by a B\&B approach over an instance of the UKP can be hard to predict, and is not very dependent on the magnitude of~\(n\) and~\(c\), but more dependent on the item distribution.
...
The objective of MTU2 is to improve MTU1 runtime when it is used in very large instances (e.g. up to 250,000 items).
...
The two main factors that motivated the creation of MTU2 were: 1) for the majority of the instances used in the period\footnote{For an example, one of the datasets of the paper that introduced MTU2 was analyzed in Section~\ref{sec:martello_uncorrelated}.}, an optimal solution is comprised of a very small number of the most efficient item types; 2) for some of those same instances, sorting of the whole items list was more expensive than solving the instance with MTU1.

The explanation of the inner workings of the MTU1 (Section~\ref{sec:mtu1}) should make it easier to understand how solving UKP with MTU1 can require less time than the sorting phase, for instances with the characteristic above mentioned (i.e. only the most efficient items are present in an optimal solution).
...
If the coefficient of two items is summed, the result is a coefficient that encodes the weight and profit value of the solution comprised by the two items. 
...
If this is not done, the algorithm cannot backtrack the items that comprise the optimal solution after finding the optimal solution value.}.
