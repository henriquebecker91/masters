...
This appendix follows the sections described in Section \ref{sec:experiments} (Experiments and Analysis).
...
t <- breq %>% mutate(n = as.numeric(get_n(filename))) %>%
  select(algorithm, n, internal_time) %>% group_by(algorithm, n) %>%
  summarise(avg = round(mean(internal_time, na.rm = TRUE), digits = 2),
            sd = round(sd(internal_time, na.rm = TRUE), digits = 2),
            min = round(min(internal_time, na.rm = TRUE), digits = 2),
            max = round(max(internal_time, na.rm = TRUE), digits = 2),
            ter = sum(!is.na(internal_time)))
xtable(t, auto = TRUE, caption = "Results of the BREQ 128-16 Standard Benchmark", tabular.environment="longtable", floating = F)
@

%\section{CSP Knapsack Subproblem Experiment}
%\section{Effect of running tests in parallel}
%\section{Experiment using the BREQ 128-16 Standard Benchmark}
