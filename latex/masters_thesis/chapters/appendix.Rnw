...
The only difference between UKP and these other KP variants is that UKP does not impose a bound on the available quantity of each item type.
...
The best bounds known for the optimal solution value of those two problems are the optimal solution value for their continuous relaxation.
The tightest formulation for BPP and CSP has an exponential number of columns and, because of this, is solved by using the column generation approach~\cite{gg-61}.
...
Esta seção trata das classes de instâncias usadas na literatura e nos experimentos apresentados neste trabalho.
Essas instâncias são diferenciadas principalmente pela distribuição dos seus items, ou seja qual a relação entre o peso e o lucro de cada item.
Neste trabalho não foram utilizadas instâncias com uma distribuição de items sem correlação (peso e lucro de cada item escolhidos aleatoriamente).

\subsubsection{Instâncias do PYAsUKP}

O conjunto de instâncias proposto em~\cite{pya}, e reutilizado na comparação apresentada em~\cite{sea2016}, será referenciado como `instâncias do PYAsUKP'.
...
\subsection{Problemas de avaliação gerados a partir do BPP/CSP}
...
Para uma mesma instância do BPP/CSP, o número de instâncias do PMR geradas, e o lucro dos itens de uma instância, pode variar baseado na escolha da solução ótima.
...
Um survey recente sobre o BPP e o CSP reuniu instâncias da literatura, e também propôs novas~\cite{survey2014}.
O número total de instâncias em todos conjuntos de instâncias apresentados no survey é de 5692.
O autor dessa dissertação escolheu 10\% dessas instâncias para os experimentos realizados nesse trabalho.
Essa fração das instâncias foi escolhida aleatoriamente dentre instâncias do mesmo conjunto ou, em conjuntos maiores, os mesmos parâmetros de geração.
As 596 instâncias do BPP/CSP selecionadas estão disponíveis no repositório GitHub do autor\footnote{As instâncias podem ser encontradas em \url{https://github.com/henriquebecker91/masters/tree/8367836344a2f615640757ffa49254758e99fe0a/data/selected_csp_inst}, e o código usado para resolver a relaxação pode ser encontrado no mesmo repositório (\url{https://github.com/henriquebecker91/masters/tree/8367836344a2f615640757ffa49254758e99fe0a/codes/cpp}). O código pode ser compilado executando  \emph{make bin/cutstock} no diretório.
...
\section{Instâncias BREQ}

O \emph{Bottom Right Ellipse Quadrant} (abreviado como BREQ, e que pode ser traduzido como `Quadrante de Elipse Inferior Direito') é um tipo de distribuição de items proposto pelo autor dessa dissertação.
...
Essa distribuição foi criada para ilustrar que diferentes distribuições de itens favorecem diferentes abordagens e, consequentemente, a escolha de conjuntos de instâncias (ou especificamente, a distribuição dos items) define o que é considerado o melhor algoritmo.
...
A conversão de instâncias do PMR em instâncias de outras variantes do problema da mochila (mochila 0-1 e mochila com limite de items) não é usada pois costuma resultar em perda de desempenho.

Algoritmos de programação dinâmica para o PMR tem um pior caso pseudo-polynomial de \(O\)(\(nc\)) (onde \(n\) é o número de itens, e \(c\) a capacidade da mochila), e um uso de memória de \(O\)(\(n + c\)).
Os algoritmos de programação dinâmica que são usados nos experimentos são: UKP5 e EDUK.
...
Todas as quatro implementações falharam em solucionar cerca de metade das instâncias antes do limite de tempo de 1000 segundos por instância.
Entre as implementações do MTU1 (C++ e Fortran) não há significativa diferença.
Entre as implementações do MTU2, pela forma como foi implementada a ordenação dos items, a implementação em C++ tem alguma vantagem, em especial para instâncias do tipo subset-sum.
Para mais informações, consultar a tabela \ref{tab:times_mtu} que se encontra nesse apêndice.

O terceiro experimento consiste na execução de todos os algoritmos mencionados na seção \ref{} sobre as cem instâncias do \emph{BREQ 128-16 Standard Benchmark}. 
Os resultados demonstram que algoritmos de B\&B, ou que possuem cálculo de limites (\emph{bounds}), tem ampla vantagem sobre algoritmos que utilizam unicamente programação dinâmica, considerando esse conjunto de instâncias.
...
O quarto experimento consiste na execução do UKP5, MTU1 e o CPLEX para solucionar os problemas de avaliação gerados a partir de instâncias do CSP/BPP.
Os resultados mostram que o CPLEX não é competitivo para solução dos problemas de avaliação.
Quando o MTU1 e o UKP5 são usados para solucionar os problemas de avaliação, na maioria das instâncias, solucionar todos os problemas de avaliação de uma determinada instância do CSP/BPP toma menos de um segundo.
Cerca de cinquenta instâncias do conjunto de instâncias do BPP/CSP exigem mais do que um segundo para que o UKP5 solucione todos os problemas de avaliação gerados pela instância (até um máximo de 50 segundos).
Quando o CPLEX e o MTU1 são usados para solucionar os problemas de avaliação dessas cinquenta instâncias mais difíceis, não é possível solucionar a instância em menos do que o limite de tempo de dez minutos.
...
O paralelismo aqui citado se refere a execuções independentes (i.e. sobre diferentes instâncias), em cores isolados, com o propósito de reduzir o tempo necessário para realizar um benchmark. 
...
O UKP5 é mais afetado por esse efeito que o PYAsUKP, o que atribuimos ao maior use de memória e, consequentemente, maior disputa pela cache compartilhada.
Todos os outros experimentos foram executados de forma serial, para evitar esse ruído.
...
Além da evidência trazidas pelos experimentos de que: o pior caso do B\&B pode ocorrer sobre problemas de avaliação (\emph{pricing problems}); a escolha entre soluções ótimas para um problema de avaliação altera consideravelmente o número de problemas de avaliação gerados subsequentemente (para uma mesma instância do BPP/CSP); algoritmos do PMR que fazem uso intensivo da memória apresentam alteração nos tempos quando executados concorrentemente porém em \emph{cores} isolados; a conversão do lucro dos problemas de avaliação de ponto flutuante para inteiro não causa perda significativa do valor da solução ótima da instância do BPP/CSP subjacente. 
