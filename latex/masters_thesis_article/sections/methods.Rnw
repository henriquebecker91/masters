...
MTU1 is B\&B algorithm for the UKP~\citep{mtu1}. %with worst-case space complexity \(O(n)\)\cite{mtu1}.
In MTU1, the items are sorted by efficiency.
Its branching scheme has \(\sum^{n}_{i=d} c' / w_d \)

By virtue of being depth-first, it only needs to keep the current solution, and systematically explores the search space
