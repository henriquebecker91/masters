...
The GREENDP does not use the best item~\(b\), but interrupts the DP at each~\(w_b\) capacity positions to verify if the DP can stop, and the remaining capacity filled with copies of \(b\). 
...
The authors' implementations of TSO and GREENDP include the same improvement added by the authors to the revisited OSO and described in \autoref{sec:oso_and_sol_dom}.
...
The UKP5 algorithm proposed in~\cite{sea2016} was found to be very similar to the TSO\footnote{
  ...
  The only improvement of UPK5 over TSO is the small change described in \autoref{sec:oso_and_sol_dom}, which the authors included in all algorithms it was applicable.}
...
also, the algorithm in~\cite{gg-63} seems to have been abandoned by its authors in favor of the OSO. %Also, \cite{cabot}~was behind a paywall.
...
In~\cite{green_improv} it is implied that GREENDP is an improved version of the algorithm proposed in~\cite{turnpike}, so only GREENDP was included.
...
The reasoning for not including \emph{uncorrelated} and \emph{weakly correlated} instance is presented in the end of the section.
...
The Bottom Right Ellipse Quadrant (BREQ) is an items distribution proposed by the authors in~\cite{ukp_hb_mastersthesis}. %and first described in REF\_MASTER\_THESIS.
