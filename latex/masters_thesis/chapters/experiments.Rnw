...
After the publication of \cite{sea2016}, the author come to perceive that algorithm's times were affected by executing multiple runs in parallel, \emph{even if each run was being executed in a exclusive/isolated core}. We credit this effect to the fact that the L3 memory cache is often shared between cores. Some experiments were run to discern the magnitude of this effect.
...
The two computer setting will be refered as \verb+notebook+ and \verb+desktop+. The \verb+notebook+ setting is the same used in \cite{sea2016}. The \verb+notebook+ configurations included: Intel\textsuperscript{\textregistered} Core\textsuperscript{TM} i7-4700HQ CPU @ 2.40GHz; there were 8GiB RAM available (\(2 \times\) DIMM DDR3 Synchronous 1600 MHz) and three levels of cache (L1d: 32K and L1i: 32K, L2: 256K, L3: 6144K, only the L3 is shared between cores). The CPU had Intel\textsuperscript{\textregistered} HyperThreading, where 4 physical cores simulate 8 virtual cores. This technology was disabled for the tests. The \verb+desktop+ configurations included: Intel\textsuperscript{\textregistered} Core\textsuperscript{TM} i7 5820k @ 3.3GHZ; there were 16GiB RAM available (\(2 \times\) Crucial Ballistix Sport DDR4 SDRAM 2400 MHz) and three levels of cache (L1i: 32K and L1d: 32K, L2: 256K, L3: 15MiB, only the L3 is shared between cores). This CPU also had Intel\textsuperscript{\textregistered} HyperThreading, where 6 physical cores simulate 12 virtual cores. This technology was also disabled for the tests in this CPU.

The observant reader will perceive the the computers setting have different amounts of cores (four for \verb+notebook+ and six for \verb+desktop+). During the experiments, in both computers, one core was left to run the OS and three cores were isolated. In the parallel runs the three isolated cores were used, in the serial run only one of them. The author believes this choice made the comparison fairer.
...
For the same computer, all runs of one algorithm were finished before starting the first run of the other algorithm. This is specially important for the parallel runs, as this means that each algorithm has only competed for cache with other runs of the same algorithm.
...
The result would be that more time is added to the PYAsUKP total time because of this effect than time is added to the UKP5 total time (in absolute numbers), making the comparison unfair to PYAsUKP (this is, unless we are interested on how the algorithms behave in a parallel setting).
...
We will see that this considerable absolute difference don't end up benefiting UKP5 in the analysis, as the PYAsUKP Serial TT is about 15.33 times greater than the UKP5 Serial TT, but the PYAsUKP Parallel TT is about 12.10 times bigger than UKP5 Parallel TT.
...
The Figure \ref{fig:sd_env_infl} present the Standard Deviation (SD) values computed over each ten runs for the same algorithm, computer, mode and instance.
...
The comparison at \cite{sea2016} was between UKP5 and PYAsUKP, here the exact same dataset is used, and one extra algorithm is compared. This experiment is complemented by the one presented at next section, that compares two implementations of both MTU1 and MTU2 over a reduced version of the same dataset. The reduced version was used because of time restraints, executing only over 10\% of the instances was sufficient to know that the methods weren't competitive. The OCaml code (PYAsUKP) was compiled with the flags suggested by the authors of the code for maximum performance.

\begin{figure}[h]
\caption{Benchmark using PYAsUKP's dataset; the UKP5, MGREENDP and EDUK2 algorithms; and no timeout.}
\begin{center}
...
The test was made with the reduced PYAsUKP benchmark (see Section \ref{sec:pya_dat_red_pya_dat}). We can see that the MTU1 implementations had a very small variation, with the C++ version being slight faster.
...
Consequently, four versions of UKP5 were used in the tests, for all combinations of profit type (the original floating point, or the converted integer), and sorting (sorting by non-increasing efficieny, or not sorting, that's the same that the items ordered by increasing weight).

The CPlex was already being used for solving the master problem, taking this in account, the author choose to use it to solve the pricing problem too.
...
The author choose to abide to the last suggestion and, in the experiment described at this section, the pricing subproblem is always solved exactly.

The timeout used in those experiments was of 600 seconds. This choice was based in the number of CSP's instances, and that each one of them had the potential for generating thousands of pricing problems. However, after the results, the choice of timeout was clearly too conservative. The author plan to remake the experiments with a greater timeout before publishing a paper based in this thesis.

The dual values of the CSP master model can be negative or zero (non-positive).
Such non-positive values can break code optimized with the assumption that all items have a positive profit.
Consequently, in the implementations of this experiment, those items are removed before solving the pricing problem with a non-CPLEX solver (CPLEX have no problem with such nonpositive profits).
The indexes of the items in the solutions of the pricing problems are remaped to their old values before returning the solution to the master problem.
The same process of remaping is already done for algoriths that change the items' order.
The time taken by sorting, conversion and remaping procedures is accounted in the times taken by the UKP's algoriths to solve the pricing subproblems.
...
Let's examinate Figure \ref{fig:csp_total_time}.
...
Two facts should be immediatly obvious: 1\textsuperscript{st}) using CPlex to solve the pricing subproblems isn't really competitive; 2\textsuperscript{nd}) for the majority of the CSP's instances in the benchmark, solving their continuous relaxation with a non-CPlex method is very fast (takes less than a second).
...
There's a small cluster of points between \(x > 450\) and \(x < 550\), with \(y < 0.1\).
...
The author don't think it's wise to draw strong conclusions using the runs that lasted less than a second.
...
First, becomes clear that the B\&B approach isn't viable for larger/harder instances, as its exponential worst-case times make the problem untractable.
...
Such small difference don't explain the gap displayed in the graph.
...
Third, seems like executing all computation using integers is slight faster than using floating point profit values, but the difference is barely noticeable.
...
The Figure \ref{fig:percentage_knap_subproblem} allows for a better visualization of what was just said.
...
Also, seems clear that when MTU1\_CUTSTOCK used large amounts of time, this was consequence of the MTU1 method taking too much time to solve the pricing subproblems.
...
In the Figure \ref{fig:percentage_knap_subproblem}, where \(x > 550\), we can see that there's a rise of the relative time taken to solve the pricing subproblem instances with UKP5.
...
Yet, we can see that using CPlex to solve the pricing subproblems seems to make solving the master problem slight easier.
Such behaviour seems to be related to the differences in the number of iterations (or master/pricing problems) solved. A topic that will be explored in the next sections.
...
In Figure~\ref{fig:relative_iter_count}, it's possible to see that the methods had solved different numbers of pricing subproblems for the same instance.
...
The choice of optimal solution will affect the master problem, that will generate a slight different pricing subproblem.
...
The effect don't seems to be associated with the B\&B approach, as MTU1 don't exhibit the same effect.
...
The UKP5 variants always return an optimal solution with the smallest weight possible. This translates to generating patterns with the greatest possible waste (the gap between optimal solution weight and the knapsack capacity equals to the waste in a cutting pattern).
...
The author found that differences between knapsack solutions because of precision loss, followed by the cascade effect, are common.
However, there was only one outlier, so the precision loss alone don't explain the outlier.
...
The Section \ref{sec:diff_it_count} has further discussion on how small changes to the algorithm changes the iteration count, and except by this outlier, this don't seem to affect the final result.
The Section \ref{sec:diff_it_count} was written based on the analysis made while trying to (unsuccessfuly) find the origin of the outlier.

\subsection{Differences in the number of pricing subproblems solved for very similar methods}
\label{sec:diff_it_count}

In Figure \ref{fig:comp_num_iter}, we can see the relative difference of the number of pricing subproblems solved between any two versions of UKP5 that share one trait and differ in the other. The two traits are: the type used for the profit values (floating point or integer); and if the items were sorted by efficiency or not. It's interesting that such small variations of the same algorithm can yield significant differences to the numbers of pricing subproblems generated. 
...
Some algorithms were available, but weren't used at this benchmark. Those are: MGREENDP, MGREENDP1, MTU2 and EDUK/EDUK2 (PYAsUKP).
...
MGREENDP1 is very inneficient, as already shown by the experiment of Section \ref{sec:breq_exp}.
The MTU2 algorithm wasn't used because it was designed for large UKP instances, and calls MTU1 over small numbers of items, what would end up with times similar to MTU1 for many instances.
...
This wasn't done because of the following reasons: many knapsacks are small and solved very fast, and in those cases the PYAsUKP timing report zero or innacurate values; also, the solutions returned by PYAsUKP would affect the next knapsacks generated, so the comparison would be unfair.
