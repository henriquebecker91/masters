...
Therefore, were generated 10 instances for each possible combination of: \(w_{min} \in \{10^3, 5\times10^3, 10^4, 5\times10^4, 10^5\}\); \(w_{max} \in \{5\times10^5, 10^6\}\) and \(n \in \{10^3, 2\times10^3, 5\times10^3, 10^4\}\), totaling 400 instances. Each instance had a random capacity in the range \([5\times10^6; 10^7]\).
...
Twenty instances were generated with each combination of \(n = 5\times10^3\), \(\alpha \in \{5, -5\}\) and \(w_{min} \in \{10^4, 1.5\times10^4, 5\times10^4\}\). More twenty instances were generated with each combination of \(n = 10^4\), \(\alpha \in \{5, -5\}\) and \(w_{min} \in \{10^4, 5\times10^4, 11\times10^4\}\). Totalling 240 instances.
...
Two hundred instances were generated for each combination of \(n = \{2\times10^4, 5\times10^4\}\) and \(w_{min} = \{2\times10^4, 5\times10^4\}\). Totalling 800 instances. Each instance had a random capacity in the range \([w_{max}; 2\times10^6]\).
...
Five hundred instances were generated for each combination of \(n = w_{min} \in \{5\times10^3, 10^4, 2\times10^4, 5\times10^4\}\). Totalling 2000 instances. Each instance had a random capacity in the range \([w_{max}; 1000\overline{n}]\).
...
Two hundred instances were generated for each combination of \(w_{min} = 10^4\), \(n \in \{10^4, 5\times10^4, 10^5\}\). Additional five hundred instances were generated with \(w_{min} = 5\times10^3\) and \(n = 5\times10^4\). Totalling 1100 instances.
...
The dataset described in the last five subsections, and totalling 4540 instances, will be called for the rest of this work: \emph{the PYAsUKP's dataset}. In each of the five smaller datasets, for the same combination of parameters, the number of instances generated was always perfectly divisible by ten. The dataset comprised by one tenth of the PYAsUKP's dataset, following the same distribution, and totalling 454 instances, will be called for the rest of this work: \emph{the reduced PYAsUKP's dataset}.
...
It's important to remember that, in this work, our objective isn't to solve CSP but its continuous relaxation. The only change this makes to the model above is that \ref{eq:csp_x_integer} is restricted to \(\mathbb{R}\), not \(\mathbb{N}_0\).
