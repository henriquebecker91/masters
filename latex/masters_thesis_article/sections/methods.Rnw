...
\textcolor{blue}{MTU1 is a B\&B algorithm for the UKP~\citep{mtu1}. %with worst-case space complexity \(O(n)\)\cite{mtu1}.
...
\textcolor{blue}{A description of the recursion, states, and stages in which the DPs for the UKP are based follow.
...
\textcolor{blue}{The naïve DP algorithm for the UKP~\cite[p.~311]{tchu}, an improved version of it presented in~\cite[p.~221]{garfinkel} and the OSO~\cite[p.~15]{gg-66} are all \(O(nc)\) DP algorithms similar to each other. % These three DP algorithms are \(O(nc)\).
...
The UKP5 algorithm proposed in~\cite{sea2016} was found to be very similar to TSO\footnote{
  ...
  \textcolor{blue}{The only improvement of UKP5 over TSO is the tiebreaker change described in \autoref{sec:oso_and_sol_dom}, which the authors included in all algorithms it was applicable.}}
and therefore only TSO was included.

\textcolor{blue}{The authors' implementation of the first algorithm proposed in~\cite{on_equivalent_greenberg} exceeded the time limits we used in the experiments, while the second algorithm does not work for all UKP instances\footnote{\textcolor{blue}{The authors' implementations of both algorithms were made available at \url{https://github.com/henriquebecker91/masters/blob/e2ff269998576cb69b8d6fb1de59fa5d3ce02852/codes/cpp/lib/greendp.hpp}.}}.
...
\textcolor{blue}{The optimal solution of BREQ instances is often in the first fraction of the search space examined by B\&B algorithms. 
...
These datasets amount to 6195 instances\textcolor{blue}{, all made available in~\cite{Delorme2018}}. %\url{http://or.dei.unibo.it/library/bpplib}. 
