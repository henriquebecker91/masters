...
gen_graph_all_alg_discrete_x_no_tso <- function(csv) {
  csv <- filter(csv, algorithm != 'terminating_step_off')
  alg_labels <- c(
    'cpp-mtu1'             = 'MTU1 (C++)',
    'cpp-mtu2'             = 'MTU2 (C++)',
    'eduk'                 = 'EDUK',
  ...
  p <- ggplot(csv,
    ...
    scale_colour_manual(name = "Algorithm",
                       labels = alg_labels,
                       values = c('cpp-mtu1' = 'black', 'cpp-mtu2' = 'darkgrey', 'eduk' = 'black', 'eduk2' = 'darkgrey', 'ordered_step_off' = 'darkgrey', 'mgreendp' = 'black')) +
    scale_shape_manual(name = "Algorithm",
                       labels = alg_labels,
                       values = c('cpp-mtu1' = 16, 'cpp-mtu2' = 16, 'eduk' = 17, 'eduk2' = 17, 'ordered_step_off' = 15, 'mgreendp' = 4)) +
    ylab('Time to solve (seconds, log10 scale)') +
    xlab('Instance size (number of items, log2 scale)') +
    theme(legend.position = 'bottom')
  #ggsave('realistic_random.pdf', width = 0.80*210, height = 0.5*297, unit = 'mm')
...
The run times of both step-off algorithms and GFDP become almost identical as the size of the input grows. % TODO: consider removing the three and presenting only one of them
...
If the bounds stop the DP early then GFDP run time falls in the gradual-growth group. Otherwise, the run time is similar to OSO/TSO run time for the same instance.
