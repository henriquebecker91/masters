...
The applied use of the UKP chosen by the author to be focused in this work was: the pricing subproblem generated by solving the continuous relaxation of the set covering formulation for the classic BPP and CSP using the column generation approach. A summary about this use was already given in section \ref{sec:motivation}, together with the explanation of its relevance. In this section, the technical details about the relationship between BPP/CSP and the UKP will be given, allowing the reader to understand how such instances of the UKP are generated and why is necessary to solve them.
...
An instance of the CSP problem consists in: \(m\) different sheet sizes; each sheet size \(i\) has an unidimensional size \(w_i\) and a demand \(d_i > 0\), that needs to be satisfied; to satisfy the demand it's necessary to cut sheets of the desired size from master rolls of size \(c\), where \(c\) is bigger than any sheet size. It's assumed that there's a sufficient amount of master rolls to fill all demands and, as such, the instance don't define a number of master rolls available. The objective of the problem is to find a way to fill all demands while using the smallest possible number of master rolls. If one or more sheets are cut from a master roll, that master roll is considered used, and remaining space is considered waste.

% SWAP N AND M, THEN N WILL BE THE NUMBER OF different sizes/items
The previously mentioned `set covering' formulation for BPP and CSP is a tight formulation proposed in \cite{gg-61}. This formulation eliminated the problems of the classic formulation, that was loose and had too many symmetric solutions. However, the new formulation had a potentially exponential number of variables. The new formulation needed to compute all possible cutting patterns, i.e. all combinations of sheet sizes can be cut from a single master roll. Each cutting pattern \(j\) of the possible \(n\) cutting patterns was represented by a variable \(a_{ij}\) that stored the amount of sheets \(i\) that make part of the cutting pattern \(j\).

The column generation approach consists in avoiding the enumeration of all \(n\) cutting patterns. The problem's formulation is initialized with a small set of cutting patterns computed in polynomial time. This reduced problem is called the \emph{master problem}. An optimal solution for the master problem is computed, and a subproduct of this computation (the dual variables of the linear programming model), is used as input for a pricing problem. The solution of this pricing problem is the cutting pattern that will contribute the most if added to the master problem.
...
\begin{align}
  maximize & \sum_{j=1}^n x_j \label{eq:csp_objfun}\\
subject~to & \sum_{j=1}^n a_{ij} x_j \geq b_i,~~~\forall i \in \{1,...,m\}  \label{eq:csp_demand}\\
           & x_j \in \mathbb{N}_0,~~~\forall i \in \{1,...,m\} \label{eq:csp_x_integer}
\end{align}

The author didn't choose to remove already existing columns
The author decided for using the classic method for generating the patterns
The author didn't divide the groups in high demand and low demand as gg-66, but this is a detail of implementation that refer to the master problem, and don't affect UKP very much

% Non-algorithmic details of implementation go on the experiments section
%The master problem was implemented using CPLEX
%The code with the master and 

Before ending this section, it's impotant to correct a false information published in \cite{sea2016}. 
