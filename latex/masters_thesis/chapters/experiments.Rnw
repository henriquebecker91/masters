...
ukp_time_comp_plot <- function (data, legend) {
  ...
  data$language <- sapply(data$language, (function (f) { factor(gsub("cpp-mtu[12]", "C++", f)) }))
  data$language <- sapply(data$language, (function (f) { factor(gsub("fmtu[12]", "Fortran", f)) }))
  data$language <- sapply(data$language, (function (f) { factor(f, levels = c('C++', 'Fortran')) }))
  data$algorithm <- sapply(data$algorithm, (function (f) { factor(gsub(".*1", "MTU1", f)) }))
  data$algorithm <- sapply(data$algorithm, (function (f) { factor(gsub(".*2", "MTU2", f)) }))
  data$algorithm <- sapply(data$algorithm,(function (f) { factor(f, levels = c('MTU1', 'MTU2')) }))
...
p1 <- ukp_time_comp_plot(mtu1, 'none') + ggtitle("MTU1 (Fortran vs C++)")
