...
GREENDP does not use the best item~\(b\) in the DP, but interrupts the DP at each~\(w_b\) capacity positions to check if the DP can be stopped and the remaining capacity filled with copies of \(b\). 
If two or more items share the greatest efficiency, GREENDP check does not work, and it is the same as the ordered step-off.
