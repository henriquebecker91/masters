...
As BPP and CSP are very similar, and instances of one problem can be converted to instances of the other (similarly to 0-1 KP and BKP), the author will explain the relationship only in terms of the CSP.
...
However, as a consequence, the SCF needs to compute all possible cutting patterns, i.e. all combinations of sheet sizes that can be cut from a single master roll.
