...
In this chapter, some approachs and algorithms for solving UKP will be discussed.
...
The results of the experiments are many times aligned with was said in last section, and more developed here, that some approaches favor some item distributions.
...
The Dynamic Programing (DP) approach is the oldest found in the literature review (Section \ref{sec:prior_work}).
Its worst-case time complexity of is \(O(nc)\) (pseudo-polynomial).
...
Predictable in the sense that's easier to predict a reasonable time interval for solving an instance based in the characteristics just mentioned, than it's with other approaches.

The DP wost-case space complexity is \(O(n + c)\), what can be considerable more than other appoeaches (that don't allocate memory linear to \(c\)). However, the space needed can be reduced by many optimizations. Some of those optimizations are: using a periodicity bound as explained in Section \ref{sec:periodicity}; using modular arithmetic to reduce \(c\) to \(w_{max}\) in at least one array (see \cite[p.~17]{gg-66}); using a binary heaps instead of arrays (can be smaller than \(c\) if \(w_{min}\) is sufficiently big).

Also, the DP approach often gives an optimal solution for each capacity smaller then \(c\). However, some space optimizations can remove such feature.

\subsection{The naive algorithm}

In this work, the author sometimes refer to the basic (or naive) DP algorithm for the UKP.
The naive algorithm is an algorithm specifically designed for the UKP, but that apply basically no optimization.
...
The notation is basically the already introduced in Section \ref{sec:formulation}.
The letter \(y\) will be used in this and other algorithms to denote an arbitrary capacity value.
This implementation of the algorithm consider that the items \(i \in \{1,~...,~n\}\) are ordered by non-decreasing weight (i.e. \(w_1 \leq w_2 \leq ... \leq w_n\)). The arrays will always be base zero, and the items list base one.
...
While it does not seems to be much of an improvement, the use of the \(i \leq d[y - w_i]\) test eliminates solution simmetry.
...
As already mentioned, the author of this work proposed UKP5 in \cite{sea2016}, thinking it was novel.
The UKP5 algorithm was tought as an improvement of the Garfinkel's DP Algorithm presented in last section. 
...
However, if we check the chapter notes, there's the comment ``6.4: The recursion of this section is based on Gilmore and Gomory (1966). See Exercise 21 for a variation that will be more efficient for some data sets.''.
The algorithm presented in section 6.4 was a version of the algorithm in \cite{gg-66} with \emph{many} of its relevant optimizations removed, and in exercise 21 is expected of the reader to recreate \emph{one} of those optimizations based on hints given at the exercise.
The author of this work believes that this fact went unnoticed by previous authors that cited the book.
The book didn't featured the exercises' answers.
...
\begin{enumerate}
  ...
  \item \textbf{Time/memory tradeoff}: the test \(w_i \leq y\) from the algorithm in~~\cite{garfinkel} was removed in cost of more O(\(w_{max}\)) memory;
...
For example, if \(\{3, 2\}\) is dominated, then \(\{3, 2, 2\}\) and \(\{3, 2, 1\}\) will never be generated by UKP5, but \(\{3,2,3\}\) or \(\{3,2,5\}\) could yet be generated (note that, in truth, are the equivalent \(\{3,3,2\}\) and \(\{5,3,2\}\) that will be generated).
...
The author of this work, however, is partial to the algorithm's description to be found in \cite[p.~223]{book_ukp_2004}.
Some basic ideas used by EDUK were already exposed by a simple and functional-oriented algorithm proposed in \cite{algo_tech_cut}\footnote{The author of this work tried to implement this simple and functional-oriented algorithm in Haskell and in C++. Both codes had a very poor performance and weren't even considered for the experiments. The author of this work admit that the reason of the poor performance could be the poor quality of his implementations. The C++ code can be accessed in \url{https://github.com/henriquebecker91/masters/blob/663324e4f071b5bca22ab5301e29273b9db88a41/codes/cpp/lib/eduk.hpp}, and the Haskell code can be accessed in \url{https://github.com/henriquebecker91/masters/blob/f5bbabf47d6852816615315c8839d3f74013af5f/codes/hs/ukp.hs}.}.
Before the EDUK2 was proposed, EDUK was considered by some the state-of-the-art DP algorithm for the UKP.
...
Consequently, the author of this work recommends the use of the version available here\footnote{The repository of this master's thesis: \url{https://github.com/henriquebecker91/masters/blob/f5bbabf47d6852816615315c8839d3f74013af5f/codes/ocaml/pyasukp_mail.tgz}.}.
The author of this work was given access to the latter version by Vincent Poirriez who kindly answered an e-mail in January 11, 2016.

The author must confess that he don't have full understanding of the EDUK algorithm inner workings.
...
For some instances, specially the ones with big \(w_{min}\) and \(c\) values, this sparse representation allows for saving time and memory.
...
Each slice of the item list, beginning with by the ones with smallest items, is then processed.
...
After evaluating all slices of the item's list, and if \(c > w_{max}\), then EDUK begins to evaluate slices of the capacity values, using the items weren't eliminated by the collective dominance tests.
After each one of those capacity slices, the EDUK algorithm test the items for threshold dominance, potentially removing some of them from the global list of undominated items. 
If this list ends up consisting only of the best item (that can never be threshold dominated), then the EDUK stops and fill the remaining capacity with copies of the best item.
...
If the upper bound of any subdivision is smaller than or equal to the global lower bound, then the solutions of that subdivision of the search space don't need to be examined (i.e. the best solution know at the moment is already equal to or better than any solution that can be found in that subdivision of the search space).
...
The datasets used in the seventies had large \(n\) and \(c\) values, and items' distributions that made easy exclude large portions of the search space with the greedy lower bound solution (the uncorrelated distribution is the perfect example).
...
The MTU1 sorts the items in non-increasing efficiency order before beginning, such ordering is needed to assure the correctness of the bounds and, consequently, of the algorithm itself.
...
Consequently, the tree does not need to be enumerated explicity, the current node/solution is sufficient to know which solutions should be tried next.
...
The MTU2 call the MTU1 internally to solve the UKP, it can be seen as a wrapper around MTU1 to avoid unnecessary computational effort.
...
The MTU1 investigates first the regions of the search space with the biggest ammount of the most efficient items. 
...
Consequently, the ideia is to guess a \(k\) value, where \(k \leq n\), find and sort only the \(k\) most efficient items, and then solve this tentative core problem (in the specific case of MTU2, the solver used is MTU1).
...
Some B\&B algorithms were never implemented or deeply studied by the author of this work, but are cited here for completeness.
In \cite{cabot}, a B\&B algorithm for UKP is presented.
In that period, the B\&B algorithms were often refered as `enumeration algorithms'.
As already said in Section \ref{sec:prior_work}, Cabot's algorithm was indirectly compared with MTU1 in \cite{mtu1}, and MTU1 have shown better times.
...
Three years after proposing this algorithm, the same authors wrote \cite{gg-66}, taht focused the one-dimensional and two-dimensional knapsack problems.
This last paper has presented four variations of a DP algorithm for the UKP, but don't appear to have mentioned to the old B\&B algorithm.
...
The algorithm presented in \cite{green_improv}, it's an improvement of the ordered step-off from \cite{gg-66}.
Consequently, it's very similar to UKP5.
The author don't know if it could be defined as a hybrid, but a good definition for it would be a `DP algorithm with bounds'.
The algorithm wasn't named at the paper, and will be called GREENDP for the rest of the paper.
The implementation of the GREENDP made by the author of this work, and used in the experiments section, will be called MGREENDP (Modernized GREENDP, in the sense that the algorithm now use loops instead of the \verb+goto+ directive). 
...
A lower bound for solutions using the best item is computed by combining the current best solution of the DP with as much copies of the best item as possible.
An upper bound for solutions not using the best item is computed by combining the current best solution of the DP with a pseudo-item that would fill the entire capacity gap and have the same efficiency as the second best item (it could also be seen as solving a continuous relaxation of the UKP without the best item, and only for the remaining capacity).
...
This approach using bounds is fundamentally different from the periodicity check used by UKP5 (or the periodicity check used by the `terminating step-off'). For example, the bounds strategy makes it competitive in the BREQ instances in a way periodicity checks don't make UKP5 competitive (see experiments of the Section \ref{sec:breq_exp}). However this seems to have an impact in other families of instances (see experiments of the Section \ref{sec:pya_exp}).
...
The EDUK2 algorithm was proposed in \cite{pya}, and it is an hybridization of the EDUK (a DP algorithm) with MTU2 (a B\&B algorithm).
The author gives here a quick overview of the hybridization, more details can be found in the paper above mentioned.
As with EDUK, the author don't claim to fully comprehend the EDUK2 internals, and it's only summarizing what's said in the original paper.
...
This B\&B algorithm has the possibility of choosing between three bound formulas, and stops after exploring \(B = 10,000\) nodes (of the implicit enumeration tree).
...
The implementation of GREENDP1 by the author of this work, and used in the experiments section, will be called MGREENDP1 (Modernized GREENDP1, in the sense that the algorithm now use loops instead of the \verb+goto+ directive)).
...
The algorithm is similar to GREENDP1, but present some improvements as: the encoding tries to create the smallest coefficients as possible; the enumeration of the solutions (or `consistency check') can be done using the Djikstra's algorithm (shortest path, \(O(n + \alpha^2)\)), or a method for solving group problems developed in \cite{glover1969integer} (\(O(n \times \alpha)\)).
...
The author find both approachs to be similar to DP, in the sense that they are more like ways to understand (or interpret) the problem and then solve it by a correspondent DP algorithm, and less like B\&B that's a completely different approach when compared to DP.
