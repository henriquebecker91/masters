...
Four experiments are presented in this section.
...
The differences of the experiment presented here are: GREENDP is considered, UKP5 is replaced by an improved ordered step-off, all runs are serially executed.
...
The third experiment ????? BREQ 128-16 Standard Benchmark.
...
% TODO: duplicated with paragraphs in the beggining of the section, decide what to keep and how
...
The experiment presented in this article includes one extra algorithm (GREENDP), and does not execute any runs in parallel.
