...
This way, for any UKP instance where \(c \leq y\) we can reduce instance capacity by \(max(1, \lceil(c-y^{*})/w_b\rceil)\times w_b\). After solving this instance with reduced capacity we can add \(max(1, \lceil(c-y^{*})/w_b\rceil)\) copies of \(b\) to the optimal solution to obtain an optimal solution for the original instance.
...
The new solution are stored at \(g\) and \(d\), they replaces solutions already stored if the new solution has the same weight but a greater profit value.
...
In other words, if \(\{3, 2\}\) is dominated, then \(\{3, 2, 2\}\) and \(\{3, 2, 1\}\) are not generated by UKP5, but \({3,2,3}\) or \(3,2,5\) can yet be generated.
...
What we are saying is that, as the algorithm executes, some items will stop being used to create new solutions, as the four known dominance relations were being applied to the item list, with little, if any, overhead. We credit the surprising performance of UKP5 to this behaviour.
...
If this assumption is false, then the described condition will never happen, and the algorithm will iterate until \(y = c\) as usual. The correctness isn't affected.

There's an \emph{else if} test at line \ref{if_new_lower_bound_end}. 
If \(g[y + w_i] = g[y] + p_i \land i < d[y + w_i]\) then \(d[y] \gets i\). 
This may seem unnecessary, as appears to be an optimization of a rare case, where two solutions made from different item multisets have the same weight and profit. 
