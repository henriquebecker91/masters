...
\begin{itemize}
\item Solutions are symmetric when they are represented differently, but are equivalent.
\item For example: \(\{ 2, 1, 3, 1 \} \equiv \{ 3, 2, 1, 1 \}\)
...
\begin{itemize}
\item Naive DP algorithms solve the UKP for every capacity in~\([1,c]\) (or \([w_{min},c]\)).
\item Ex.: for the weights 7, 10, 5; no solution will have weight exactly: 6, 8, 9, 11, 13, 16, ...
...
\item This test can be removed if we use \(w_max\) extra memory.
\end{itemize}
}

\frame{\frametitle{Periodicity}
\begin{itemize}
\item The item with the biggest profit-to-weight ratio is often called ``best item''.
...
\item Implements all five previously presented techniques
\begin{itemize}
