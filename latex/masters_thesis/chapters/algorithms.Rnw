...
SKIP DIDI
% see pages 5, 6 and 7 of the UnboundedKnapsackProblemDynamicProgrammingRevisited
% get book with ritt
% check Unbounded Knapsack Problem: New Results (a conference paper with more information about the methods)
% pages 5, 6, and 7 of the AHybrid...
...
The author does not intend to gives a complete introduction to the B\&B approach, but he belives a quick overview is in order.
The B\&B approach can be seen as an improvement of the brute-force approach.
The brute-force approach would consist of exhaustively examinating all solutions in the search space.
...
In the worst case, a B\&B algorithm can't eliminate a significative portion of the search space by the use of bounds and then, consequently, it needs to examinate all search space.
...
The B\&B algorithms for the UKP often aren't affect by the magnitude of \(c\), however they can be affect by how close \(c\) is from a capacity that can be entirelly filled by copies of the best item.
