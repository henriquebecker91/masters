...
GREENDP does not generate solutions with the best item during dynamic programming, this strategy weakens solution dominance (i.e. solutions that are not a subset of an optimal solution are needlessly generated), what the authors believe to be the reason of GREENDP performance drop.
...
In summary, for the instances of the PYAsUKP dataset, TSO and MGREENDP often need less time than EDUK2 to solve an instance.
EDUK2 solves some instances in less time than TSO and MGREENDP, but it takes much more time than UKP5/MGREENDP to solve the 
