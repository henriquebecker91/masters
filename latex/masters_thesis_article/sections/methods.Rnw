...
The UKP5 algorithm proposed in~\cite{sea2016} was found to be very similar to the TSO\footnote{
  The authors of this article reinvented one algorithm from~\cite{gg-66} and published a paper calling it UKP5 while believing it was novel~\cite{sea2016}.
  The authors would like to apologize to the academic and scientific community for such disregard.
  The only significant improvement of UPK5 over TSO is the small change described in \autoref{sec:oso_and_sol_dom}, which the authors included in all algorithms it was applicable.}
