...
rr_csv$n <- sapply(rr_csv$filename, rr_get_n)
...
\section{Realistic Random Dataset}

\begin{figure}[h]
\caption{TODO: write caption.}
\begin{center}
<<realistic_random_figure,echo=FALSE>>=
...
ggplot(rr_csv,
       aes(x = n * (0.80 + 0.50*(as.numeric(algorithm) - 1)/6),
           y = internal_time,
           color = algorithm,
           shape = algorithm)) + 
  #geom_jitter() +
  #stat_bin2d(binwidth = 0.25, geom="text", aes(label=..count..), position = position_dodge(0.7)) +
  #scale_fill_gradient(guide = FALSE) + # remove count legend
  ...
  scale_x_continuous(trans = "log2",
                     breaks = c(2^10, 2^11, 2^12, 2^13, 2^14, 2^15,
                                2^16, 2^17)) +
  scale_colour_manual(name = "Algorithm",
                      labels = c('cpp-mtu1', 'cpp-mtu2', 'eduk', 'eduk2', 'ordered_step_off', 'terminating_step_off', 'mgreendp'),
                      values = c('black', 'darkgrey', 'black', 'darkgrey', 'black', 'darkgrey', 'black')) +   
  scale_shape_manual(name = "Algorithm",
                     labels = c('cpp-mtu1', 'cpp-mtu2', 'eduk', 'eduk2', 'ordered_step_off', 'terminating_step_off', 'mgreendp'),
                     values = c(16, 16, 17, 17, 15, 15, 4)) +
  ylab('Time to solve (seconds)') +
  xlab('Instance size (number of items)') +
...
<<realistic_random_table,echo=FALSE,results='asis'>>=
