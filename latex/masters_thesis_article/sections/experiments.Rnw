...
All runs were executed in serial order (i.e. two algorithm executions did not coexist), as the authors found that parallel executions effected the run times (even if each isolated core only hosted one run at each time)\cite[p.~87]{ukp_hb_mastersthesis}.
...
alg_labels <- c(
  ...
  'eduk'                 = 'EDUK',
  'eduk2'                = 'EDUK2',
  'ordered_step_off'     = 'Ord. S.',
  'terminating_step_off' = 'Term. S.',
  'mgreendp'             = 'GREENDP'
)

rr_t$algorithm <- sapply(rr_t$algorithm, function(x) alg_labels[x])
rr_xt <- xtable(rr_t)
...
In this version GREENDP is considered, UKP5 is replaced by the terminating step-off, and all runs were executed serially.
The exact same 4540 instances were used.

\begin{figure}[H]
\caption{Benchmark using PYAsUKP dataset; the Terminating Step-off, GREENDP and EDUK2 algorithms; no timeout.}
\begin{center}
<<pya_fast_fig>>=
fast_plot <- function(t) {
  ...
  shape_conv <- c('T.S.' = 15, 'EDUK2' = 16, 'MGREENDP' = 17)
...
The subset-sum (S.S.) plot in \autoref{fig:pya_fast} does not show the run times of the MGREENDP algorithm.
MGREENDP bound computation does not work if the two most efficient items of an instance have the same efficiency (what always happens at subset-sum instances).
MGREENDP without its bound computation is very similar to the terminating/ordered step-offs, and so the authors found to be redundant to execute it over S.S. instances.
...
This experiment was performed with the reduced PYAsUKP benchmark (see Section~\ref{sec:pya_dat_red_pya_dat}).
% TODO: put this below in methods?
The Fortran codes used were the original implementation with all 32 bits integers or float variables/parameters replaced by their 64 bits counterparts. 

The C++ and Fortran77 implementations of the MTU1 algorithm have no significant differences.

The MTU2 implementations differed in the algorithm used to partially sort the items.
The original algorithm described in~\cite{mtu2} did not specify the exact method for performing this partial sorting.
The original Fortran77 implementation uses a complex algorithm developed by one of the authors of MTU2 in~\cite{partial_sort_martello} to find the k\textsuperscript{th} most efficient item in an unsorted array (and then sort).
The C++ implementation uses the \verb+std::partial_sort+ procedure of the standard C++ library \verb+algorithm+.
The C++ implementation also checks if the entire vector is sorted before starting to execute the partial sorts, and it sorts by non-increasing efficiency and if tied by non-decreasing weight.
The Fortran77 implementation only sorted by efficiency.
% TODO: add this in the appendix, or in methods
...
table_rows_csp <- function(t) {
  latex(
	...
	(n = 1) + count_na*total_iter +
        na_mean*#Justify('r')*
          (Format(sffsc(2))*hex_sum_master_prob_time + Format(sffsc(2))*pricing_total_time + total_iter + Format(digits=1)*(pricing_total_time/total_iter)),
      data = t
    ),
    mathmode = F,
...
cat('Dataset  & N & finished & \\makecell{Master\\\\time} & \\makecell{Total\\\\pricing\\\\time} & \\makecell{Average\\\\number of\\\\sub-problems} & \\makecell{Pricing\\\\time\\\\per\\\\subproblem}\\\\\\\\')
#cat('\\hline\n')
...
% TODO: keep only the three algorithms presented in the table, if the image
% is kept
\begin{figure}[htb]
...
Some algorithms were available, but were not used in this experiment, those are: GREENDP, MTU2 and EDUK/EDUK2 (PYAsUKP).
